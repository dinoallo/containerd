...
func NewPathTrie(fullPath string) *PathTrie {
	return &PathTrie{
...
func (trie *PathTrie) newPathTrie(fullPath string) *PathTrie {
	return &PathTrie{
...
func (trie *PathTrie) Get(key string) any {
	node := trie
...
func (trie *PathTrie) Put(key string, value any) bool {
	node := trie
	var (
		fullPath string = "" // track full path for child nodes
	)
	for part, i := trie.segmenter(key, 0); part != ""; part, i = trie.segmenter(key, i) {
		fullPath += part
		...
		if child == nil {
			...
			child = trie.newPathTrie(fullPath)
...
func (trie *PathTrie) WalkPath(key string, walker WalkFunc) error {
	...
	if trie.value != nil {
		if err := walker("", trie.fullPath, trie.value); err != nil {
			return err
		}
	}
	for part, i := trie.segmenter(key, 0); ; part, i = trie.segmenter(key, i) {
		...
		if err := walker(k, trie.fullPath, trie.value); err != nil {
			return err
		}
...
func (trie *PathTrie) walk(key string, walker WalkFunc) error {
	if err := walker(key, trie.fullPath, trie.value); err != nil {
		return err
	}
