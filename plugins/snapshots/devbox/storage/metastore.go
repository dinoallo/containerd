...
// drivers. Driver implementations are responsible for starting and managing
// transactions using the defined context creator. This storage package uses
// BoltDB for storing metadata.
package storage
...
// Snapshot holds the metadata for an active or view snapshot transaction.
// The ParentIDs hold the snapshot identifiers for the committed snapshots this
...
// process crashes from causing inconsistent metadata state.
...
func (ms *MetaStore) TransactionContext(ctx context.Context, writable bool) (context.Context, Transactor, error) {
	ms.dbL.Lock()
	if ms.db == nil {
		db, err := bolt.Open(ms.dbfile, 0o600, nil)
		if err != nil {
			ms.dbL.Unlock()
...
// TransactionCallback represents a callback to be invoked while under a
// metastore transaction.
type TransactionCallback func(ctx context.Context) error

// WithTransaction is a convenience method to run fn while holding a metastore
// transaction. If fn returns an error or the transaction is not writable, the
// database transaction will be discarded.
